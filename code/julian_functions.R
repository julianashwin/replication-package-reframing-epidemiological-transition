...
# Create dataframe with mortality rates for each year
create_mortality_df <- function(all_jmortality_df, loc_name = "Global", start_year = 2021, 
  ...
  if (loc_name == "Regions"){
    mortality_df <- all_jmortality_df %>%
  ...
  if (growth_transitions){
    ...
    cause_names <- names(select(all_jmortality_df, mortality:last_col()))
      ...
      for (yy in (max(start_year, latest_year)+1):end_year){
        ...
        for (gg in region_names){
          ...
          add_rates <- as.numeric(trans_props[1,gg])*as.matrix(all_mortality_df[which(
            all_jmortality_df$location == gg & all_mortality_df$year == max(all_mortality_df$year)),cause_names])
...
forecast_dalys <- function(population_df, fertility_df, mortality_df, disability_df, 
  ...
  if (project_100plus){
    # Make sure the rates don't exceed 1 (100,000 per 100,000 (i.e., 100%))
...
compare_forecasts <- function(population_df, fertility_df, mortality_df, disability_df, loc_name = "Regions",
  ...
  dalys_df <- pop_baseline %>% 
    ...
    mutate(pop_diff = population_new - population_base, 
           daly_diff = daly_new - daly_base,
           pop_diff_mort = population_mort - population_base, 
           daly_diff_mort = daly_mort - daly_base,
           pop_diff_dis = population_dis - population_base, 
           daly_diff_dis = daly_dis - daly_base) %>%
    arrange(location, year, age)
  ...
  dalys_yly <- dalys_df %>% 
    ...
    summarise(across(c(population_base, population_new, population_mort, population_dis, 
                       ...
                       pop_diff, pop_diff_mort, pop_diff_dis, daly_diff, 
                       daly_diff_mort, daly_diff_dis,
                       LE_base, LE_new, LE_mort, LE_dis, HLE_dis, HLE_new, 
                       HLE_mort, HLE_dis), sum)
              )
